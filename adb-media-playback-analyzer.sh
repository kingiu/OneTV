...
echo "=== ADB 媒体播放日志分析器 ==="
echo "用于捕获和分析Android设备媒体播放过程中的错误和警告日志"
echo ""

PACKAGE_NAME="com.guozhi.onetv"
LOG_FILE="/tmp/media_playback_log.txt"
ANALYSIS_FILE="/tmp/media_playback_analysis.txt"
...
# 捕获日志（实时监控）
adb logcat -v time "*:E" "*:W" | grep -i -E "media|player|video|audio|exoplayer|mediacodec|decode|encode|render" > "$LOG_FILE" &
...
# 编解码问题
echo "1. 编解码问题:" >> "$ANALYSIS_FILE"
grep -i -E "mediacodec|decode|encode|codec" "$LOG_FILE" | head -20 >> "$ANALYSIS_FILE"
echo "" >> "$ANALYSIS_FILE"

# 资源加载失败
echo "2. 资源加载失败:" >> "$ANALYSIS_FILE"
grep -i -E "load|resource|network|http|https" "$LOG_FILE" | head -20 >> "$ANALYSIS_FILE"
echo "" >> "$ANALYSIS_FILE"

# 权限问题
echo "3. 权限问题:" >> "$ANALYSIS_FILE"
grep -i -E "permission|denied|access" "$LOG_FILE" | head -20 >> "$ANALYSIS_FILE"
echo "" >> "$ANALYSIS_FILE"

# 硬件加速问题
echo "4. 硬件加速问题:" >> "$ANALYSIS_FILE"
grep -i -E "hardware|accel|gpu|render" "$LOG_FILE" | head -20 >> "$ANALYSIS_FILE"
echo "" >> "$ANALYSIS_FILE"

# ExoPlayer 相关问题
echo "5. ExoPlayer 相关问题:" >> "$ANALYSIS_FILE"
grep -i "exoplayer" "$LOG_FILE" | head -20 >> "$ANALYSIS_FILE"
echo "" >> "$ANALYSIS_FILE"
...
if grep -i "exoplayer" "$LOG_FILE" | grep -i "error" > /dev/null; then
    echo "🔧 ExoPlayer 问题建议:"
    echo "- 检查 ExoPlayer 版本兼容性"
    echo "- 实现播放器错误处理和恢复机制"
    echo "- 检查播放器配置是否合理"
    echo ""
fi
