// 测试复杂影视资源匹配功能（模拟多个危险关系剧集的场景）
import { resourceMatcher } from './services/resourceMatcher';
import { SearchResult } from './services/resourceMatcher';

// 模拟搜索结果数据（包含多个"危险关系"剧集和其他混淆项）
const mockResults: SearchResult[] = [
  ...
  },
  {
    ...
    episodes: ['https://example.com/ep1.m3u8', 'https://example.com/ep2.m3u8', 'https://example.com/ep3.m3u8'],
...
// 测试函数
async function testComplexMatcher() {
  console.log('=== 测试复杂影视资源匹配功能 ===\n');
  ...
  // 测试带年份的查询
  console.log('\n=== 测试带年份的查询 ===');
  const queryWithYear = '危险关系 2024';
  console.log(`测试查询: "${queryWithYear}"`);
  
  const processedWithYear = resourceMatcher.processSearchResults(queryWithYear, mockResults);
  console.log(`\n匹配结果数量: ${processedWithYear.matches.length}`);
  
  console.log('\n=== 匹配结果详情 ===');
  processedWithYear.matches.forEach((match, index) => {
    console.log(`\n${index + 1}. ${match.result.title}`);
    console.log(`   分数: ${match.score.toFixed(2)}`);
    console.log(`   年份: ${match.result.year}`);
  });
...
// 运行测试
testComplexMatcher().catch(console.error);
