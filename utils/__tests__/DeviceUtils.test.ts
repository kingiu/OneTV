...
describe("DeviceUtils", () => {
  ...
  describe("getDeviceType", () => {
    ...
    it("应该在宽度 >= 768 且 < 1024 时返回 tablet", () => {
      mockedDimensions.get.mockReturnValue({ width: 768, height: 1024, scale: 2, fontScale: 1 });
      expect(DeviceUtils.getDeviceType()).toBe("tablet");
    });

    it("应该在宽度 < 768 时返回 mobile", () => {
      mockedDimensions.get.mockReturnValue({ width: 375, height: 812, scale: 2, fontScale: 1 });
  ...
  describe("isLandscape", () => {
    it("应该在横屏模式下返回 true", () => {
      mockedDimensions.get.mockReturnValue({ width: 812, height: 375, scale: 2, fontScale: 1 });
